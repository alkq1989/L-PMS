...
public:
	...
	int *readAndEncodeInput(char *fileName, int& totalMsgWords) {
		...
		if (totalLen < 0)
			...
			cout << "Read " << totalLen << " chars from file " << fileName
					<< endl;
			return encodeData(strings, totalMsgWords); // 就是main函数中返回的b参数
		}
	}
        
	void loadConfigFromBuffer(int *b) { // 加载b中的参数
		// read alphabet
		char *bc = (char*) b;
		int n = b[0];
		int sigmaLen = b[n + 1];
	...
	void printMotifs(set<MyString>& motifs) {
		std::cout << "Motifs:" << endl;
                string ss;
                ofstream outMotif("/home/lin/Documents/outMotif.txt");
                for(set<MyString>::iterator it=motifs.begin();it!=motifs.end();it++){
                    outMotif<<decodeString(it->s,mc.L,sigma)<<endl;
                }
		for (set<MyString>::iterator it = motifs.begin(); it != motifs.end();++it) {
                    ss=decodeString(it->s,mc.L,sigma);
                   if(filterMotif(ss, getSubString(), mc.L,mc.d)){
                       continue;
                   }else {
                       std::cout << decodeString(it->s, mc.L, sigma)<<"   第"<<++n<<"个"<<std::endl;
                   }
		}
	}
...
private:
	...
	void populateThresholds(int totalLen, MotifConfig& mc) {
		...
		double minStackSizeSquared = max(4.0, (2 * mc.d - 2) * logSigma - log2(avgStringLen) + 8);
...
//		}
	...
	int *encodeData(vector<string>& strings, int& totalMsgWords) {
		...
		for (int i = 0; i < mc.n; ++i)
			b[i + 1] = strings[i].length(); //+n

		// write alphabet
		b[mc.n + 1] = sigma.length(); //现在长度 n+1
		char *bc = (char*) b;
