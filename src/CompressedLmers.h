...
public:
	typedef uint16 comprWordType; //无符号16位整数
...
//		if (i < L)
	...
	int HamDistGeneratedVsStored(const comprWordType *ca,const int storedItemIndex) {
...
//		if (i < L)
	...
	void compressLmer(const char *l, comprWordType *cl) { //cl为无符号16位整数类型的数组
		int i = 0;
		for (; i + symbolsPerWord <= L; i += symbolsPerWord) {
			comprWordType w = 0; //无符号16位整数
